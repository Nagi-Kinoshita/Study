...
row_index=1;
ranges = [0, 24; 0, 18; 0, 3.5; 0, 1.4; 0, 1.2; 0, 1.7; 0, 2.2; 0, 2.2; ...
          0, 2.8; 0, 5.5; 0, 0.7; 0, 1.4; 0, 1.3; 0, 1.5; 0, 1.9; 0, 1.8; ...
          0, 4.7; 0, 13; 0, 18; 0, 18; 0, 18; 0, 20; 0, 24; 0, 24];
...
while(1)
    ...
    rng('shuffle');
    % 配列を初期化
    LED_stim = zeros(1, size(ranges, 1));
    
    % 指定範囲内でランダムな値を生成
    for i = 1:size(ranges, 1)
        LED_stim(i) = ranges(i, 1) + (ranges(i, 2) - ranges(i, 1)) * rand();
    end

    % XYZを計算
    XYZ = calculate_X_Y_Z_lum(LED_stim);%LED_stimulus_spectral_intensity2.csvになってたら修正する
    if(XYZ(2)<0.41)
        continue;
    end
