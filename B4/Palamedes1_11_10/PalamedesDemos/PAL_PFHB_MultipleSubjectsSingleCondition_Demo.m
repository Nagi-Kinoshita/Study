...
a_gen(1:Nsubj) = .25*randn(1)+randn(1,Nsubj)*.5;
b_gen(1:Nsubj) = .5+.25*randn(1)+randn(1,Nsubj)*.25;
g_gen = .5*ones(Nsubj);
l_gen = [.01 .02 .03 .07 .01 .04];    %different generating lapse rate for each subjects
...
for s = 1:Nsubj
    ...
    paramsGen = [a_gen(s), 10.^b_gen(s), g_gen(s), l_gen(s)];   %parameter values [alpha, beta, gamma, lambda] (or [threshold, slope, guess, lapse]) used to simulate observer
