...
EEW_answer=zeros([44 22]);%最終的にはzeros([40 22])にする。
random_stim=zeros([1 22]);
Test_stim=readmatrix('Test_stim.csv');

Name="Kinoshita2";%被験者の名前
Exp_Number=1;%実験ごとに変える、1と3の時は上限に合わせてもらって、2と4の時は下限に合わせてもらう
...
for i=1:3
    ...
    while 1
        ...
        rng(count+i+x);
        random_stim_number=randperm(44);
        for j=1:10
            rng(count+i+x+j);
